...
- **Shared services domain**: `services.hl` (configurable via `HOMELAB_SHARED_DNS_DOMAIN`)
...
#### Shared Services
Format: `<service>.services.hl`

Examples:
- `vault.services.hl` → 10.147.17.100 (VIP)
- `registry.services.hl` → 10.147.17.101 (VIP)
- `minio.services.hl` → 10.147.17.102 (distributed endpoint)

#### Proxy Shortcuts (Memorable URLs)
Format: `<service>.homelab.internal` (uses base domain)

Examples:
- `vault.homelab.internal` → Proxy → `vault.services.hl`
...
# Shared services zone (services.hl)
services.hl             SOA, NS records
├── vault.services.hl   A record → 10.147.17.100
├── registry.services.hl A record → 10.147.17.101
└── minio.services.hl   A record → 10.147.17.102

# Base zone for shortcuts (homelab.internal)
homelab.internal                    SOA, NS records
├── vault.homelab.internal          CNAME → vault.services.hl
...
# Primary DNS (Site A)
IP: 10.147.17.5
Role: Primary authoritative for pickers.hl, services.hl, homelab.internal
Zones: pickers.hl (primary site), services.hl, homelab.internal
...
Zones: sheila.hl (secondary site), services.hl (replicated), homelab.internal (replicated)
...
Proxy (Traefik) inspects hostname
  ↓
Routes to: vault.services.hl (10.147.17.100)
```
...
```
vault.homelab.internal      CNAME   vault.services.hl
argocd.homelab.internal     CNAME   proxy.services.hl
grafana.homelab.internal    CNAME   grafana.services.hl
```
...
1. Create a new application in Authentik:
   ...
   - External URL: `http://dns.homelab.internal` or `http://dns-primary.services.hl`
...
**Access Flow:**
1. Navigate to `http://dns.homelab.internal` or `http://dns-primary.services.hl`
...
metadata:
  ...
  namespace: dns-system
  annotations:
    external-dns.alpha.kubernetes.io/hostname: "dns-primary.services.hl"
spec:
  type: LoadBalancer
...
# Test shared service
dig vault.services.hl @10.147.17.5
...
dig vault.services.hl @10.147.17.5
dig vault.services.hl @10.147.17.25
...
curl -X POST "http://10.147.17.5:5380/api/zones/records/add" \
  -H "Authorization: Bearer ${TECHNITIUM_API_TOKEN}" \
  -d "domain=myservice.services.hl" \
  -d "type=A" \
...
1. **Shared services** use `.services.hl` with VIPs
